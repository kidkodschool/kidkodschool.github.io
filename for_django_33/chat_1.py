...
class ChatTest(TestCase):
    ...
    def test_chat_send_message(self):
        ...
        self.assertEqual(len(obj), 1, msg='Ошибка создания записи в чат - Не удалось создать запись')
        from django.utils import timezone
        now = timezone.now()
        self.assertEqual(obj[0].user.username, 'basic', msg='Ошибка создания записи в чат - Неправильное сохранение в поле user')
        self.assertEqual(obj[0].text, 'test', msg='Ошибка создания записи в чат - Неправильное сохранение в поле text')
        self.assertEqual(obj[0].date.date(), now.date(), msg='Ошибка создания записи в чат - Неправильное сохранение в поле date')
